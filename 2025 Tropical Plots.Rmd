...
ggsave(plot = bubbleplot_mifish_Bayside_tropicals, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2025/bubbleplot_mifish_Bayside_tropicals.jpg",
...
"/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2025/bubbleplot_mifish_Month_tropicals.jpg", width = 10, height = 3, units = "in")
...
ggsave(plot = bubbleplot_mifish_Month_trawlonly_tropicals, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2025/bubbleplot_mifish_Month_trawlonly_tropicals.jpg", width = 8, height = 3, units = "in")
...
ggsave(plot = bubbleplot_mifish_Month_eelgrass_tropicals, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2025/bubbleplot_mifish_Month_eelgrass_tropicals.jpg", width = 8, height = 3, units = "in")
...
bubbleplot_mifish_Month_clam_tropicals <- ggplot(mifish_df_month_clam_tropicals, aes(x = factor(sites, levels = sitelevels), y = fct_rev(`Taxon.CommonName`))) + 
geom_point(aes(size=ifelse(Abundance==0, NA, Abundance), fill = `Taxon.CommonName`), color = "black", pch = 21) + 
scale_size_area(breaks = c(1000,10000,100000,1000000,10000000))+ 
xlab("")+ 
ylab("")+
...
guides(fill="none") + 
facet_grid(~Month_f, drop = TRUE, scales = "free", space='free')
bubbleplot_mifish_Month_clam_tropicals

# Save

ggsave(plot = bubbleplot_mifish_Month_clam_tropicals, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2025/bubbleplot_mifish_Month_clam_tropicals.jpg", width = 8, height = 3, units = "in")
...
# No species found at oyster reefs


### Artifical structure site
# First average replicates from each site

mifish_df_month_AS <- mifish_df %>% filter(sites %in% c(48)) %>% group_by(sites, Month_f, `Taxon.CommonName`) %>% dplyr::summarize(Abundance = mean(Abundance)) 
mifish_df_month_AS

# Subset out tropical species

mifish_df_month_AS_tropicals <- mifish_df_month_AS %>%
    filter(Taxon.CommonName %in% c(
        "Caranx hippos (Crevalle jack)",
        "Fistularia tabacaria (Bluespotted cornetfish)",
        "Sphyraena borealis (Northern sennet)",
...
mifish_df_month_AS_tropicals

# No species found at artificial structure
...
species_counts_2025_mifish_tropicals <- mifish_df_tropicals %>% group_by(Month) %>% summarise(UniqueSpeciesCount = n_distinct(`Taxon.CommonName`), .groups = "drop") %>% complete(Month = all_months, fill = list(UniqueSpeciesCount = 0))

# Plot

sp_richnesss_habitat_month_mifish_tropicals_plot <- ggplot(species_counts_2025_mifish_tropicals, aes(x = Month, y = UniqueSpeciesCount)) + 
geom_line(size = 1, alpha = 0.8) + 
geom_point(size = 2, alpha = 0.8) + 
scale_x_continuous(breaks = seq(7, 9, by = 1), limits = c(7, 9)) + 
scale_y_continuous(breaks = seq(0, 6, by = 2), limits = c(0, 6)) + 
labs(x = "Month", y = "Species Richness", title = "2025 Species Richness; Tropicals") + 
theme_minimal() + 
...
ggsave(plot = sp_richnesss_habitat_month_mifish_tropicals_plot, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2025/sp_richnesss_habitat_month_mifish_tropicals_plot.jpg", width = 5, height = 4, units = "in")
...
ggsave(plot = species_counts_by_bayside_mifish_tropicals_plot, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2025/species_counts_by_bayside_mifish_tropicals_plot.jpg", width = 2.5, height = 4, units = "in")
...
ps_2025_mifish <- metagMisc::phyloseq_filter_sample_wise_abund_trim( phylo_2025_mifish, minabund = 0, relabund = FALSE)

# Summarize total abundances for each species across the whole dataset

ps2025_mifish_abundance <- data.frame(otu_table(ps_2025_mifish)) %>%
  rowSums() %>%
  as.data.frame() %>%
...
ps2025_mifish_abundance <- ps2025_mifish_abundance %>%
  mutate(RelativeAbundance = TotalAbundance / sum(TotalAbundance))
  
# Add back taxonomic info

tax_info <- as.data.frame(tax_table(ps_2025_mifish))
tax_info <- tax_info %>%
...
ps2025_mifish_abundance <- ps2025_mifish_abundance[, colSums(!is.na(ps2025_mifish_abundance)) > 0]

# Subset out tropial species

ps2025_mifish_abundance_tropicals <- ps2025_mifish_abundance %>%
    filter(Taxon.CommonName %in% c(
        "Caranx hippos (Crevalle jack)",
        "Fistularia tabacaria (Bluespotted cornetfish)",
        "Sphyraena borealis (Northern sennet)",
...
ps2025_mifish_abundance_tropicals

# Find total relative abundance of all species. It's 0.0059

sum(ps2025_mifish_abundance_tropicals$RelativeAbundance)

# Plot relative abundance

mifish_tropicals_barplot_ra <- ggplot(ps2025_mifish_abundance_tropicals, aes(
    y = RelativeAbundance,
...
"/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2025/mifish_tropicals_barplot_ra.jpg",
