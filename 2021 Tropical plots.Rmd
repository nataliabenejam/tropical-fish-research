---
title: "2021 Tropical Plots"
output: html_document
date: "2025-12-12"
---

## Code is from Liz Suter's notebook (<https://github.com/lizsuter/shirp-edna/blob/main/2024-eDNA-plots.nb.Rmd>) modified for 2025 mifish tropical samples
...
asv_count_mifish <- read.csv("/Volumes/NB_2/filtered_eDNA_data/2021_mifish/mifish_2021_glommed_asv_table.csv", header=TRUE, stringsAsFactors=FALSE)
...
asv_taxonomy_mifish <- read.csv("/Volumes/NB_2/filtered_eDNA_data/2021_mifish/mifish_2021_glommed_tax_table.csv", header=TRUE, stringsAsFactors=FALSE)
...
sample_metadata_mifish <- read.csv("/Volumes/NB_2/filtered_eDNA_data/2021_mifish/mifish_2021_sample_data.csv", header=TRUE, stringsAsFactors=TRUE)
row.names(sample_metadata_mifish) <- sample_metadata_mifish$X
sample_metadata_mifish <- sample_metadata_mifish %>% select(-X)

ASV_2021_mifish = otu_table(asv_count_mifish_mat, taxa_are_rows = TRUE)
TAX_2021_mifish = tax_table(asv_taxonomy_mifish_mat) 
samples_2021_mifish = sample_data(sample_metadata_mifish)

phylo_2021_mifish <- phyloseq(ASV_2021_mifish, TAX_2021_mifish, samples_2021_mifish)
...
phylo_2021_mifish_glommed <- tax_glom(phylo_2021_mifish, "Taxon.CommonName")
mifish_df <- subset_samples(phylo_2021_mifish_glommed)
mifish_df <- psmelt(mifish_df)

# relative abund ps object

phylo_2021_mifish_glommed_ra <- tax_glom(phylo_2021_mifish, "Taxon.CommonName")
mifish_df_ra <- subset_samples(phylo_2021_mifish_glommed_ra) 
...
ggsave(plot = bubbleplot_mifish_Bayside_tropicals, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2021/bubbleplot_mifish_Bayside_tropicals.jpg",
...
"/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2021/bubbleplot_mifish_Month_tropicals.jpg", width = 10, height = 3, units = "in")
...
ggsave(plot = bubbleplot_mifish_Month_trawlonly_tropicals, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2021/bubbleplot_mifish_Month_trawlonly_tropicals.jpg", width = 10, height = 3, units = "in")
...
ggsave(plot = bubbleplot_mifish_Month_oysterreef_tropicals, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2021/bubbleplot_mifish_Month_oysterreef_tropicals.jpg", width = 8, height = 3, units = "in")
...
species_counts_2021_mifish_tropicals <- mifish_df_tropicals %>% group_by(Month) %>% summarise(UniqueSpeciesCount = n_distinct(`Taxon.CommonName`), .groups = "drop") %>% complete(Month = all_months, fill = list(UniqueSpeciesCount = 0))

# Plot

sp_richnesss_habitat_month_mifish_tropicals_plot <- ggplot(species_counts_2021_mifish_tropicals, aes(x = Month, y = UniqueSpeciesCount)) + 
geom_line(size = 1, alpha = 0.8) + 
geom_point(size = 2, alpha = 0.8) + 
scale_x_continuous(breaks = seq(7, 10, by = 1), limits = c(7, 10)) + 
scale_y_continuous(breaks = seq(0, 6, by = 2), limits = c(0, 6)) + 
labs(x = "Month", y = "Species Richness", title = "2021 Species Richness; Tropicals") + 
theme_minimal() + 
...
ggsave(plot = sp_richnesss_habitat_month_mifish_tropicals_plot, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2021/sp_richnesss_habitat_month_mifish_tropicals_plot.jpg", width = 5, height = 4, units = "in")
...
ggsave(plot = species_counts_by_bayside_mifish_tropicals_plot, filename = "/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2021/species_counts_by_bayside_mifish_tropicals_plot.jpg", width = 2.5, height = 4, units = "in")
...
ps_2021_mifish <- metagMisc::phyloseq_filter_sample_wise_abund_trim( phylo_2021_mifish, minabund = 0, relabund = FALSE)

# Summarize total abundances for each species across the whole dataset

ps2021_mifish_abundance <- data.frame(otu_table(ps_2021_mifish)) %>%
  rowSums() %>%
  as.data.frame() %>%
...
ps2021_mifish_abundance <- ps2021_mifish_abundance %>%
  mutate(RelativeAbundance = TotalAbundance / sum(TotalAbundance))
  
# Add back taxonomic info

tax_info <- as.data.frame(tax_table(ps_2021_mifish))
tax_info <- tax_info %>%
...
ps2021_mifish_abundance <- ps2021_mifish_abundance[, colSums(!is.na(ps2021_mifish_abundance)) > 0]

# Subset out tropial species

ps2021_mifish_abundance_tropicals <- ps2021_mifish_abundance %>%
    filter(Taxon.CommonName %in% c(
        "Fistularia tabacaria (Bluespotted cornetfish)",
        "Sphyraena borealis (Northern sennet)",
        "Chaetodon ocellatus (Spotfin butterflyfish)", 
...
ps2021_mifish_abundance_tropicals

# Plot relative abundance

mifish_tropicals_barplot_ra <- ggplot(ps2021_mifish_abundance_tropicals, aes(
    y = RelativeAbundance,
...
"/Volumes/NB_2/filtered_eDNA_data/Figures/Tropicals/2021/mifish_tropicals_barplot_ra.jpg",
